...
### 5.6 尝试并放弃：recover-to-walk 专家

P0 把验收做实之后，最弱的一环仍是"切回行走"（`handstand→walk` 0.686–0.76，
而每个技能单跑都 >0.95）。诊断是：走路专家的训练分布只有"站姿起步"，
示范数据里没有任何"walk token 在动作中途到达"的样本；而 §5.3.1 已证明不能
拿单技能专家去标注离线流形上的状态。于是做一个分布真正覆盖这些状态的专家。

**工具**：新增任务 `LainLab-OpenDoge-Recover-Walk`——沿用走路任务的奖励集与
速度指令，但**每一批有一半从随机跌倒姿态起步**（`fallen_probability=0.5`），
并把 `illegal_contact` 终止项换成稠密的非足端接触惩罚（否则跌倒起步第一步就
会被终止）。它的流形严格包含"任意姿态 → 行走"。

**结果：全面变差，按预先约定的 gate 回滚。**
...
**这不是指标假象，已核对过**。把 walk 的成功判据拆开看交接后 250 步：

| 策略 | 站立且迈步 | 站立（不论是否迈步） | 站立但四足全着地 | 直立度 |
|---|---|---|---|---|
| 交付版 | **0.733** | **0.895** | 0.162 | **+0.933** |
| recover-to-walk | 0.426 | 0.608 | 0.182 | +0.707 |
| `student4` | 0.552 | 0.799 | 0.246 | +0.863 |

"四足全着地"占比三者相近（0.16–0.25），所以判据里 `feet_down<4` 那条并没有
在惩罚"先站稳"；下降的是**直立度**（0.933 → 0.707），即策略确实更容易摔。

**根因有两条，第二条更重要**：

1. recover 专家优化的是**静态**跌倒姿态（getup 式分布），而不是"从倒立摔下来"
   这种带速度的动态状态，所以它并没有真正覆盖目标状态；
2. `jump → handstand` 也一起塌了（0.969 → 0.370），**而倒立专家和它的数据都没动**
   ——只改走路数据就足以扰动共享主干、伤到不相干的技能。这与 §5.4 里 PPO 微调
   撞的是同一堵墙。

**结论**：共享主干的克隆处在一个局部最优，继续做数据工程会反复把它推出去。
下一步应该改**架构**而不是数据——per-skill actor head，或 per-skill 价值头 +
按技能归一化奖励。gate 未通过，已回滚；入库 baseline 未受影响（同一检查点在恢复
后的环境里复现：walk 1.000 / 0.050、倒立 0.953、起身 0.929、跳跃 0.073）。

**保留的部分**：`LainLab-OpenDoge-Recover-Walk` 任务及其 4 个测试保留为"仪器"
（下次做架构隔离时它是现成的覆盖分布）；`StandingReset.fallen_probability` 这个
通用机制也保留——它取代了原先对 getup 的硬编码特例。统一环境里 **walk 的起步
分布已改回站姿**，与入库策略的训练分布保持一致。
...
| recover-to-walk 专家（失败，未采用） | 4096 | 4999 | 32 分 57 秒 | 5000 | **100%** |
| 行为克隆 P1（失败，未采用） | — | 30 epoch | 约 7 分 | 30 | **100%** |
