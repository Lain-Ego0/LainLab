...
1. **是 jump 数据把 walk 顶出去的**：去掉 1,024,000 条移动跳跃样本，walk 误差从
   0.0649 回到 0.0569；倒立/起身完全不受影响（数值一模一样）。
2. **0.06 这条门槛在当前代码下不可达**。walk 专家自己在统一环境里的保真值就是
   **0.059**（§4.1，单任务 0.061），walk-only 克隆 0.0588 —— 克隆已经和专家持平。
   也就是说共享主干只要有任何干扰，就必然突破 0.06。参考值 0.051 来自跳跃重设计
   **之前**的代码状态（当时统一观测 54 维、jump 是原地版），现在无法复现。
